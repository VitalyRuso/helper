...
export function AdminPage() {
  ...
  const reindex = useMutation({ mutationFn: () => api.reindex(token), onSuccess: () => rag.refetch() });
    ...
    return (
      <section className="mx-auto max-w-md px-4 py-10">
        ...
        <form className="mt-6 space-y-3 rounded-md border border-ink/10 bg-white p-5" onSubmit={submit}>
          ...
          <button className="w-full rounded-md bg-brand px-4 py-2 font-medium text-white disabled:opacity-50" disabled={login.isPending}>
            Войти
          </button>
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-10">
      <div className="flex items-center justify-between gap-4">
        ...
        <button className="rounded-md border border-ink/15 bg-white px-4 py-2 text-sm" onClick={logout}>
          Выйти
        </button>
      </div>
      ...
      <div className="mt-6 rounded-md border border-ink/10 bg-white p-5">
        ...
        {rag.isLoading && <div className="mt-3"><LoadingState /></div>}
        {rag.error && <div className="mt-3"><ErrorState error={rag.error} /></div>}
        {rag.data && !rag.data.qdrant_available && <div className="mt-3"><ErrorState error={rag.data.error ?? "Qdrant недоступен"} /></div>}
